...
export function groupTextsByPosition(results: OCRTextResult[]): GroupedText[] {
  console.log('[OCR Client] Starting text grouping with', results.length, 'items');
  
  // 按垂直位置排序
  const sortedResults = [...results].sort((a, b) => a.location.top - b.location.top);
  ...
  sortedResults.forEach((result, index) => {
    if (lastTop === -1 || Math.abs(result.location.top - lastTop) > VERTICAL_THRESHOLD) {
      currentGroup++;
      console.log('[OCR Client] Created new group', currentGroup, 'at position', result.location.top);
    }
